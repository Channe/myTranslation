...
![神秘但准确的描述了单子的装订(bind)](https://www.uraimo.com/imgs/bind.png)

考虑到这门语言依然是一个初期状态，通常尝试去理解一些特定的话题，最后你会发现许多文章都是说这门语言之前的版本，或者更糟糕的一些，内容混杂着许多版本。

有时，搜索 flatMap 的文章，甚至你会意外的发现不止一篇好文章都会在 Swift 中解释单子(Monads)。

事实上，由于缺乏丰富并且相对较新的相关概念的资料。甚至是带有一些生硬的隐喻，都不是通俗易懂的，特别是一些人使用一些难理解的方式思考问题。

在这篇简短的文章中（这是一个 [Swift  和函数式的系列文章](https://www.uraimo.com/category/functional/)），我将试着给出清晰全面的解释，如何在 Swift 2.0 中对不同的类型使用 `map` 和特别的 `flatMap` .
...
这个行为附带着单子上的 `(装订)bind` 操作，要了解更多可以阅读[这篇](http://khanlou.com/2015/09/what-the-heck-is-a-monad/)以及[这篇](http://robnapier.net/flatmap)。
